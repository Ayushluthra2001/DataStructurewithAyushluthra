...
class Solution
{
    ...
    long long solve(int coins[],int numberOfCoins,int val,int curr,vector<long long >&dp){
        ...
        for(int i=0;i<numberOfCoins;i++){
            mini=min(mini,1+solve(coins,numberOfCoins,val,curr+coins[i],dp));
        }
        return dp[curr]= mini;
    }
    ...
    {
        // your code here
        vector<long long >dp(1e3+1,-1);
        int ans=solve(coins,numberOfCoins,value,0,dp);
        if(ans>=1e8) return -1;
        return ans;
    }
};
